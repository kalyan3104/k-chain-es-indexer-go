...
def prepare_observer(shard_id, working_dir, config_folder):
    ...
    shutil.copyfile(working_dir / "k-chain-go/cmd/node/node", working_dir_observer / "node/node")
...
def generate_new_config(working_dir):
    k_chain_go_folder = working_dir / "k-chain-go" / "scripts" / "testnet"
    num_of_shards = str(os.getenv('NUM_OF_SHARDS'))

    with open(k_chain_go_folder / "local.sh", "w") as file:
...
def clone_k_chain_go(working_dir):
    print("cloning k-chain-go....")
    k_chain_go_folder = working_dir / "k-chain-go"
    if not os.path.isdir(k_chain_go_folder):
        Repo.clone_from(os.getenv('NODE_GO_URL'), k_chain_go_folder)

    repo_k_chain_go = Repo(k_chain_go_folder)
    repo_k_chain_go.git.checkout(os.getenv('NODE_GO_BRANCH'))


def clone_dependencies(working_dir):
    print("cloning dependencies")
    k_chain_deploy_folder = working_dir / "k-chain-deploy-go"
    if not os.path.isdir(k_chain_deploy_folder):
        Repo.clone_from(os.getenv('K_CHAIN_DEPLOY_GO_URL'), k_chain_deploy_folder)

    k_chain_proxy_folder = working_dir / "k-chain-proxy-go"
    if not os.path.isdir(k_chain_proxy_folder):
        Repo.clone_from(os.getenv('K_CHAIN_PROXY_URL'), k_chain_proxy_folder)


def prepare_seed_node(working_dir):
    print("preparing seed node")
    seed_node = Path.home() / "kalyan3104/testnet/seednode"
    shutil.copytree(seed_node, working_dir / "seednode")

    k_chain_go_folder = working_dir / "k-chain-go"
    subprocess.check_call(["go", "build"], cwd=k_chain_go_folder / "cmd/seednode")

    seed_node_exec = k_chain_go_folder / "cmd/seednode/seednode"
...
def prepare_proxy(working_dir):
    ...
    k_chain_proxy_go_folder = working_dir / "k-chain-proxy-go"
    subprocess.check_call(["go", "build"], cwd=k_chain_proxy_go_folder / "cmd/proxy")

    k_chain_proxy_go_binary_folder = k_chain_proxy_go_folder / "cmd/proxy"
    st = os.stat(k_chain_proxy_go_binary_folder / "proxy")
    os.chmod(k_chain_proxy_go_binary_folder / "proxy", st.st_mode | stat.S_IEXEC)

    # config.toml
    path_config = k_chain_proxy_go_binary_folder / "config/config.toml"
...
def generate_config_for_local_testnet(working_dir):
    k_chain_local_testnet_scripts = working_dir / "k-chain-go/scripts/testnet"
    subprocess.check_call(["./clean.sh"], cwd=k_chain_local_testnet_scripts)
    subprocess.check_call(["./config.sh"], cwd=k_chain_local_testnet_scripts)

    config_folder = Path.home() / "kalyan3104/testnet/node/config"
...
def main():
    ...
    # build binary k-chain-go
    print("building node...")
    k_chain_go_folder = working_dir / "k-chain-go"
    flags = '-gcflags=all=-N -l'
    subprocess.check_call(["go", "build", flags], cwd=k_chain_go_folder / "cmd/node")
